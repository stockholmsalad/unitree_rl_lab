...
# CUDA 메모리 단편화 완화 — 셸 export 여부와 무관하게 보장(반드시 torch/CUDA init 전).
# recurrent BPTT backward 에서 iter 가 진행될수록 단편화가 쌓여 OOM 나는 것 방지.
import os  # noqa: E402
...
# JE-Loco point cloud task 등록 (lazy, isaaclab 불필요) — task 목록 필터 전에 등록되어야 함.
...
# RTX 50-series(Blackwell sm_120) + torch2.7/cuDNN9.2 는 GRU/conv 커널 부재로 CUDNN_STATUS_NOT_INITIALIZED.
# cuDNN 비활성화 시 native RNN 로 폴백(동작). 근본 해결은 torch/cuDNN 업그레이드(cuDNN≥9.7).
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # convert deprecated (rsl-rl < 4.0.0) `policy` config into the new `actor`/`critic`
