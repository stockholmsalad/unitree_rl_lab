...
## 한 줄 상태 (2026-08-07)
**핵심 반전**: ICCAS "예측>재구성 결손강건" = sentinel 아티팩트, 마스킹에서 사라짐.
JEPA 살리기 두 시도 모두 무신호: **conditioning(k=5) 실패, 지평확대(k∈{5,15,27}) 무신호**.
④②+residual 로 예측은 작동 시작(skill~0.5)했으나 지평이 강건성/skill 안 올림. **메타결론:
이 과제는 proprioception 으로 결손강건성 포화 → 예측이 이길 축 없음.** 다음 = **전략 전환:
시각 load-bearing 과제(정밀 foothold/gap = Stage 2/[D]5)로.** 사용자 결정 대기.
