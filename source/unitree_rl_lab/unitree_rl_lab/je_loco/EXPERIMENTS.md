...
## 중간 판정 @ iter 5400 (2026-08-19)

`D_{jepa,recon,scratch}_s{1,2}` 6런, pilab, 1024 env, 8000 iter 목표(14.2 s/iter).

### ① 보행 품질 — DAgger 는 명백히 성공

| | Phase 3 (PPO) | **Phase 3b (DAgger)** | teacher |
|---|---|---|---|
| ep_len | 830~870 | **962~971** | 948 → 979 |
| 넘어짐 | 5.9~7.7% | **2.8~3.2%** | 3.5% |
| 자세 이탈 | 0.66~0.76 | **0.558~0.567** | 0.680~0.743 |
| mean_reward | 74~86 | **126.9~128.8** | (env 달라 직접비교 불가) |
| play 육안 | "셋 다 별로"(평지) | **레벨 3 에서 계단·gap 통과** | 통과 |

**자세가 teacher 보다 조이다**(0.56 vs 0.68). Phase 3 가 12000 iter 로 못 간 곳에 도달.

### ② A 판정 — behavior loss

| iter | 1000 | 2000 | 3000 | 4000 | **5409** |
|---|---|---|---|---|---|
| jepa | 0.0746 | 0.0642 | 0.0632 | 0.0598 | **0.0549** |
| recon | 0.0646 | 0.0699 | 0.0614 | 0.0583 | **0.0537** |
| **scratch** | **0.0903** | **0.0885** | **0.0744** | **0.0656** | **0.0601** |

개별 seed @5409 — recon 0.0535·0.0539 / jepa 0.0538·0.0560 / **scratch 0.0593·0.0609**.

- **사전학습 > scratch**: scratch 두 seed 가 사전학습 네 seed 보다 **전부** 높음(겹침 없음).
  iter 1000~5409 **전 체크포인트에서 scratch 꼴찌**. 격차 ~9%.
  단 n=2 라 무작위 확률 ≈ 1/15 = **6.7%** — 시사적이나 확정 아님.
- **jepa ≈ recon**: 0.0549 vs 0.0537 (2%). 무작위 확률 1/6 = 17% = **노이즈 범위**.

### ③ ★ 단서 — 손실 차이가 행동으로 안 넘어온다

ep_len·reward·넘어짐·err_vxy·terrain·자세 **전부 동률**(jepa 967.4/128.3/0.028 · recon
969.7/128.6/0.028 · scratch 961.8/126.9/0.030). **사전학습의 이점은 teacher 모방 정확도에만
있고 과제 성능으로는 안 넘어온다.** 논문에 반드시 병기할 것.

### ④ 경계 — 이 프로젝트 전력

"scratch 두 seed 가 전부 위" 는 **두 번 증발했던 것과 같은 모양**의 결과다(① sentinel
② 거미 자세). p≈0.067 이면 그때(p≈0.011)보다 오히려 약하다. **확정하려면 seed 증설 필수.**
다만 순서가 iter 1000~5409 내내 안정적이라는 점은 이전 두 건과 다르다.

---

# ★ play 도구 버그 2건 (2026-08-19) — 도구도 검증 대상이다

이 프로젝트는 "지표는 멀쩡한데 육안에서 걸린다" 가 세 번(정지 편법·기형 보행·거미 자세)이라
육안을 신뢰해 왔다. 그런데 이번엔 **육안 쪽 도구가 틀린** 반대 경우가 두 건 나왔다.

### 버그 1 — 증류 체크포인트가 로드되지 않음 → "가만히 서 있음"

`play_pc.py` 가 `OnPolicyRunner` 로 로드하는데:
- 증류 체크포인트 키 = `student_state_dict` / `teacher_state_dict`
- PPO 의 `load` 는 `loaded_dict["actor_state_dict"]` 를 **직접 인덱싱**(`ppo.py`)
- Distill 태스크 agent cfg 도 `actor`/`critic` 이 아니라 `student`/`teacher`

**정책이 로드되지 않은 초기 네트워크는 출력이 0 근처 → 기본 관절 자세 유지 → "가만히 서 있음".**
학습 실패와 구분 불가능한 실패 모드. 교차검증 근거는 학습 로그(reward 127.7, ep_len 969.8) —
정지 편법이면 추종 보상(weight 3.0)이 0 이라 이 값이 안 나온다.
**수정:** 알고리즘에 따라 `DistillationRunner` 분기(`get_policy()` 가 student 반환).

### 버그 2 — play 지형이 항상 레벨 0 → 평지만 보고 있었음

`scripted_terrain_levels` 는 난이도 상한을 `it = env.common_step_counter / steps_per_iter`
로 정하는데, **play 는 step_counter 가 0 부터**라 `frac=0.10` → `ceiling = max(1, round(0.10×10)) = 1`
→ `randint(0,1)` = **매 reset 마다 전 로봇 레벨 0**. play_env_cfg 의 `max_init_terrain_level`
설정이 통째로 무시된다.

**영향(중요):** 이 프로젝트의 **모든 play 육안 판정이 레벨 0~1(사실상 평지)에서 이뤄졌다.**
- Phase 3 "셋 다 별로" → **평지에서도 별로였다** = 판정이 오히려 **강해짐**
- Phase 3b "잘 걷는다"(첫 확인) → 평지 기준이었음 → 레벨 3 재확인 후 통과
- teacher play 는 영향 없음(teacher env 는 적응형 `terrain_levels_vel`)
- `eval_foothold.py` 는 원래 `curriculum.terrain_levels = None` 처리를 하고 있었다 — **play 만 누락**

**수정:** play 에서 지형 커리큘럼 해제 + `--terrain_level N` 추가(0~9, 스폰 [0,N]).

---

## 다음

1. **8000 완주** (~8~10 h). 손실 아직 완만히 하강(4~5k 구간 평균 0.0567), 순서는 안정.
2. **★ 결손 평가** — 지금 조건 차이가 미미한데, 원래 가설이 갈릴 자리는 입력 훼손 시점.
   DeFM 도 깨끗할 땐 무승부(90.14 vs 90.45), **OOD 에서만** 갈렸다(0.876 vs 0.486).
   `--degradation occlusion --dropout_levels 0,0.3,0.6,1.0` (occlusion = 구조 최대 = 추론 필요).
3. 결손 평가도 무차별이면 → **결손 학습 `D2_*`**(`raycaster_pointcloud` 에 결손 파라미터 배선).
   우리는 결손을 평가에만 썼지 **학습에 넣은 적이 없다.**
